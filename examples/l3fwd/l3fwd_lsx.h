...
processx4_step3(struct rte_mbuf *pkt[FWDSTEP], uint16_t dst_port[FWDSTEP])
{
	...
	__m128i ve[FWDSTEP];
	__m128i *p[FWDSTEP];
	const __m128i mask_eth = lsx_vbitselmaski_h(MASK_ETH);
	...
	te[0] =  __lsx_vbitsel_v(te[0], ve[0], mask_eth);
	te[1] =  __lsx_vbitsel_v(te[1], ve[1], mask_eth);
	te[2] =  __lsx_vbitsel_v(te[2], ve[2], mask_eth);
	te[3] =  __lsx_vbitsel_v(te[3], ve[3], mask_eth);
...
process_packet(struct rte_mbuf *pkt, uint16_t *dst_port)
{
	...
	te =  __lsx_vbitsel_v(te, ve, lsx_vbitselmaski_h(MASK_ETH));
